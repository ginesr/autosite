...
my $trimmed = Forms::Param::Field->new('that ');
...
my $html = Forms::Param::Field->new($snip);
is( $html->html_stripped, "This sucks\n", 'No html' );
$html->strip_html();
is( "$html", "This sucks\n", 'Removed html' );

my $html_enc = Forms::Param::Field->new($snip);
is( $html_enc->encoded, "&lt;title&gt;This sucks&lt;/title&gt;\n",
    'HTML encoded' );

my $html_dec = Forms::Param::Field->new($html_enc);
is( $html_dec->decoded, "<title>This sucks</title>\n", 'Decode HTML' );

my $email       = 'test@server.com';
my $email_field = Forms::Param::Field->new($email);

is( $email_field->is_like_email, 1, 'Is an email' );

my $one_param   = Forms::Param::Field->new('same thing');
my $other_param = Forms::Param::Field->new('same thing');
my $third_param = Forms::Param::Field->new('not same thing');

cmp_ok( $one_param, 'eq', $other_param, 'Same content' );
cmp_ok( $one_param, 'ne', $third_param, 'Not same instance' );

my $array_ref_one = Forms::Param::Field->new( ['one'] );

is( $array_ref_one->value, 'one', 'One value is flattened' );

my $multi_val = Forms::Param::Field->new( [ 'one', 'three' ] );
...
my $from_multi = Forms::Param::Field->new( $multi_val->at(0) );
